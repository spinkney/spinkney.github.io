...
transformed parameters {
  ...
  
  {
    ...
    matrix[P, P] varcov = chol2inv(cholesky_decompose( add_diag(crossprod(diag_pre_multiply(sqrt(V), x)), 1e-3  )));
...
model {
  ...
  sigma ~ inv_gamma(3.0 / 2 * N, dot_product(v,  (y - Mu)^2) / 4 + zeta * 1 / sum(v));
