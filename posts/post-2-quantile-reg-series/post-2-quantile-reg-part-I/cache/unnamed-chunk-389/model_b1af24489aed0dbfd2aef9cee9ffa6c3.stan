...
transformed parameters {
  ...
  
  {
    ...
    matrix[P, P] varcov = inverse(crossprod(diag_pre_multiply(V, x)));
    vector[P] betam = varcov * transpose(x) * diag_pre_multiply(V, y - xi / v_inv);
...
model {
  ...
  sigma ~ gamma(3.0 / 2 * N,  0.25 * sum( (y - Mu)^2 .* v_inv)  + zeta / sum(v_inv));
