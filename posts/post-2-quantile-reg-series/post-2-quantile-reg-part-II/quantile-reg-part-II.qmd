...
[Part I](/posts/post-2-quantile-reg-part-I/quantile-reg.qmd) showed the basic Bayesian quantile regression using the asymmetric Laplace distribution and augmented scheme. In this post I'm going to show an approximate likelihood, score method proposed in @score.
...
``` stan
  matrix make_w (matrix x, real q) {
    ...
    return alpha * inverse_spd(x_out) * N * 0.5; // modified here
  }
```
...
functions{
  ...
  matrix make_w (matrix x, real q) {
   ...
   return alpha * inverse_spd(x_out) * (N * 0.5);
