...
In this post I'll show a way to do quantile regression on a bounded unit interval. 
...
$$
\begin{aligned}
\ell(\mu, \theta \mid y, \tau) = & \\
& \frac{n}{2}\log\bigg(\frac{1 - \theta}{\theta} \bigg) - \frac{\Phi^{-1}(y)^\top \Phi^{-1}(y)}{2} - \frac{1}{2\theta} \sum_{i=1}^{n} \bigg(\sqrt{1 - \theta}\bigg[\Phi^{-1}(y_i) - \Phi^{-1}(\mu) \bigg] + \sqrt{\theta}\Phi^{-1}(\tau) \bigg)^2
\end{aligned}
$$
...
```{r}
data.frame(stan = fit_stan_vector$summary()[-1, 1:2], gamlss_estimates =c(t(gamlss_mtau)))
```
