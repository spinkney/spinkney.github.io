functions {
  ...
  */
  ...
  matrix cholesky_corr_constrain_ldl3_lookahead_jacobian(
      int K, vector raw, array[,] int zeros, int propagation_passes) {
    // Omega = L * diag(D) * L', with unit lower-triangular L. The
    // remaining row radius is tracked on the natural scale.
      ...
      for (j in 1:(i - 1)) {
        ...
        */
        ...
        if (current_is_zero) {
          ...
          // LDL3 transform for direct log-density/gradient parity.
          if (propagation_passes == 0) {
            low = max({-b2, -1.0 - b1});
            up = min({b2, 1.0 - b1});
          } else {
        ...
        // This is the original LDL3 change of variables and radius update.
...
transformed parameters {
  matrix[K, K] L_Omega =
      cholesky_corr_constrain_ldl3_lookahead_jacobian(
