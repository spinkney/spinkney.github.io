...
| `schur` | 4 $\to$ **5** | 224 $\to$ **193**[^schur-lookahead] | 191.9 $\to$ **202.2** |

[^schur-lookahead]: The pass-zero `schur` fit failed during initialization for seed 5, while the one-pass transform initialized all four chains. Its divergence and efficiency comparison therefore uses the four patterns that succeeded under both settings.
