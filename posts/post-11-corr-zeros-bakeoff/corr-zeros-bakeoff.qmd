...
Sometimes you know, before seeing any data, that certain correlations are exactly zero. For example, a priori knowledge that two variables are conditionally unrelated occurs in graphical models as either an explicit assumption or by pre-existing knowledge that rules out the variable coupling. Building this structure directly through the correlation matrix parameterization is what this blog post is about.

You cannot get there by sampling from the LKJ and zeroing out cells, as the result is generally not positive definite. The zeros have to be built into the parameterization itself, and how you build them in turns out to matter a great deal for the sampler. In this post I test twelve Stan [@carpenter2017] parameterizations of the same target posterior against each other. Since all twelve provably target the same distribution (and I verify this numerically), the comparison isolates exactly one thing: **sampling geometry**. Divergences, effective sample size, and cost per gradient evaluation.

The two families of solutions make opposite trade-offs, and a hybrid, an exact-constraint construction with a canonical triangular basis, is best of all. **Zero divergences in every run, the best ESS per gradient in the field, and the fastest wall clock.**
...
The two exact-constraint members deserve a closer look side by side. `tri`'s divergence count is zero in every cell of this table; the donut matches it from $\eta = 2$ up but gives back a small count at $\eta = 1$ (bracketed below). On efficiency the pattern is dimension-dependent: `tri` holds its min bulk ESS close to the 8,000-draw ceiling through $K = 15$, while the donut rides lower — but at $K = 25$, where the min runs over 157 free correlations, the donut's extra radial parameters stop costing and start paying, and its raw min ESS comes out 4--5$\times$ `tri`'s at comparable cost per gradient. Wall clock stays in seconds for both.
