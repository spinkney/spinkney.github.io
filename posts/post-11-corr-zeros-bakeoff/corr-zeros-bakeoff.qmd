...
The fix is to anchor the basis to the matrix entries. For each free column $k$ of row $i$, take $e_k$ and fill in the row's forced columns by back-substitution through the zero constraints. This is exactly the Family 1 forced-entry fill, applied to *basis vectors* instead of the sampled row. Collect these as the columns of $T$. Splitting coordinates into forced ($Z$) and free ($F$) sets gives $T_F = I$ and $T_Z = -(A_Z')^{-1} A_F'$, and orthonormalization costs one small Cholesky factor $S = \operatorname{chol}(T'T)$: the map $v = T (S')^{-1} z$ satisfies $\lVert v \rVert = \lVert z \rVert$, so the sphere-measure bookkeeping goes through untouched. The subsphere coordinates $z$ come from the row parameterization I proposed for Cholesky correlation factors on Discourse [@pinkney2025]: with $t = y'y$, $z = \sqrt{t + 2}/(t + 1) \, y$ and the diagonal is $1/(t + 1)$, with chart Jacobian $\tfrac{1}{2}(d - 2)\log(t + 2) - (d + 1)\log(1 + t)$ -- one vectorized map per row, no per-coordinate loop.
...
The two exact-constraint members deserve a closer look side by side. `tri`'s divergence count is zero in every cell of this table; the donut matches it from $\eta = 2$ up but gives back a small count at $\eta = 1$ (bracketed below). On efficiency the pattern is consistent: `tri` holds its min bulk ESS close to the 8,000-draw ceiling through $K = 15$, while the donut rides lower, and at $K = 25$, where the min runs over 157 free correlations, both settle in the 4,700--6,600 range at comparable cost per gradient, with `tri` slightly ahead. Wall clock stays in seconds for both.
...
**Anchoring the basis is not a nicety.** The literal QR implementation survives everywhere the forced family dies, but it pays for the spinning frame: at $K = 25$, $\eta = 1$ it needed 852 seconds against `tri`'s 8.9, and at $K = 15$, $\eta = 2$ it failed catastrophically, one chain pinned at 100% divergence and a min ESS of 7. (The forced family has its own catastrophic mode: `ldl3` at $K = 9$, $\eta = 2$ came back with a min ESS of 19.) `qr2`'s *raw* min ESS at $K = 25$ lands around 3,500--4,100 against `tri`'s 5,100--6,600, and it burns far more gradient evaluations to get there. Per gradient, and per second, `tri` is the class of the field: the only model at exactly zero divergences in all twenty configurations (the donut keeps a residual sub-1.5% rate at $\eta = 1$), flat wall clock, and the best ESS per gradient in 17 of the 20 cells (`qr2` edges it in three of the $K = 5$ corners).
