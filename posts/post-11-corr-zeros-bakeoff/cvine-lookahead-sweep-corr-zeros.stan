/*
  Experimental look-ahead C-vine parameterization with configurable breadth
  and finite 3 x 3 interval-propagation depth. This extends the
  equality-specialized transform in
  https://gist.github.com/spinkney/7d994fb03079c9edb8a0e83a40d2faad.
...
functions {
  ...
  */
  ...
  /*
    Perform a fixed number of symmetric Jacobi sweeps over all 3 x 3
    principal submatrices whose indices are at least first_active. Each pass
    computes all three edge projections from the bounds at the beginning of
    that pass, so finite pass counts have an order-independent meaning.

    is_assigned marks exact point coordinates: structural equalities and
    free coordinates already consumed by the sequential transform. Those
    points are validated but never moved. Unassigned intervals are
    intersected with every applicable projection.
  */
  tuple(matrix, matrix) corr3_propagate_symmetric(
      matrix lo_in,
      matrix hi_in,
      array[,] int is_assigned,
      int first_active,
    ...
    if (propagation_passes > 0 && first_active <= K - 2) {
      for (pass in 1:propagation_passes) {
        matrix[K, K] lo_old = lo;
        matrix[K, K] hi_old = hi;
        matrix[K, K] lo_next = lo;
        matrix[K, K] hi_next = hi;

        for (a in first_active:(K - 2)) {
          for (b in (a + 1):(K - 1)) {
            for (c in (b + 1):K) {
              vector[2] projected_ab = corr3_project_first(
                  lo_old[a, c], hi_old[a, c],
                  lo_old[b, c], hi_old[b, c]);
              vector[2] projected_ac = corr3_project_first(
                  lo_old[a, b], hi_old[a, b],
                  lo_old[b, c], hi_old[b, c]);
              vector[2] projected_bc = corr3_project_first(
                  lo_old[a, b], hi_old[a, b],
                  lo_old[a, c], hi_old[a, c]);

              if (is_assigned[a, b] == 1) {
                real fixed_ab = 0.5 * (lo_old[a, b] + hi_old[a, b]);
                if (fixed_ab < projected_ab[1] - validation_tolerance
                    || fixed_ab > projected_ab[2] + validation_tolerance) {
                  reject("3 x 3 propagation found assigned pair (", a, ",",
                         b, ") incompatible with variable ", c,
                         ". Fixed value=", fixed_ab, ", projection=[",
                         projected_ab[1], ",", projected_ab[2], "].");
                }
              } else {
                lo_next[a, b] = fmax(lo_next[a, b], projected_ab[1]);
                hi_next[a, b] = fmin(hi_next[a, b], projected_ab[2]);
                lo_next[b, a] = lo_next[a, b];
                hi_next[b, a] = hi_next[a, b];
              }

              if (is_assigned[a, c] == 1) {
                real fixed_ac = 0.5 * (lo_old[a, c] + hi_old[a, c]);
                if (fixed_ac < projected_ac[1] - validation_tolerance
                    || fixed_ac > projected_ac[2] + validation_tolerance) {
                  reject("3 x 3 propagation found assigned pair (", a, ",",
                         c, ") incompatible with variable ", b,
                         ". Fixed value=", fixed_ac, ", projection=[",
                         projected_ac[1], ",", projected_ac[2], "].");
                }
              } else {
                lo_next[a, c] = fmax(lo_next[a, c], projected_ac[1]);
                hi_next[a, c] = fmin(hi_next[a, c], projected_ac[2]);
                lo_next[c, a] = lo_next[a, c];
                hi_next[c, a] = hi_next[a, c];
              }

              if (is_assigned[b, c] == 1) {
                real fixed_bc = 0.5 * (lo_old[b, c] + hi_old[b, c]);
                if (fixed_bc < projected_bc[1] - validation_tolerance
                    || fixed_bc > projected_bc[2] + validation_tolerance) {
                  reject("3 x 3 propagation found assigned pair (", b, ",",
                         c, ") incompatible with variable ", a,
                         ". Fixed value=", fixed_bc, ", projection=[",
                         projected_bc[1], ",", projected_bc[2], "].");
                }
              } else {
                lo_next[b, c] = fmax(lo_next[b, c], projected_bc[1]);
                hi_next[b, c] = fmin(hi_next[b, c], projected_bc[2]);
                lo_next[c, b] = lo_next[b, c];
                hi_next[c, b] = hi_next[b, c];
              }
            }
          }
        }

        for (a in first_active:(K - 1)) {
          for (b in (a + 1):K) {
            if (is_assigned[a, b] == 1) {
              if (abs(hi_next[a, b] - lo_next[a, b])
                  > validation_tolerance) {
                reject("Assigned interval acquired width during 3 x 3 ",
                       "propagation at pair (", a, ",", b, ").");
              }
            } else if (!(hi_next[a, b] - lo_next[a, b]
                         > interval_tolerance)) {
              reject("3 x 3 propagation emptied free interval at pair (",
                     a, ",", b, "): [", lo_next[a, b], ",",
                     hi_next[a, b], "].");
            }
          }
  ...
  */
  tuple(matrix, real) cvine_bound_equality_jacobian(
    ...
    array[K, K] int pair_is_assigned = rep_array(0, K, K);
      ...
      if (i > 1) {
        for (j in 1:(i - 1)) {
          ...
          if (width > 0.0) {
            ...
            pair_is_assigned[i, j] = 1;
            pair_is_assigned[j, i] = 1;
    ...
    for (i in 1:(K - 1)) {
      for (j in (i + 1):K) {
        ...
        int lookahead_end = j + lookahead_span < K
                            ? j + lookahead_span : K;
        real lo_direct;
        real hi_direct;
        real lo;
        ...
        // Propagation sees only point values sampled before this coordinate
        // and interval bounds for this and later coordinates. It never uses
        // the current or a future raw coordinate.
        if (propagation_passes > 0 && i <= K - 2) {
          tuple(matrix[K, K], matrix[K, K]) propagated =
              corr3_propagate_symmetric(
                  lo_work, hi_work, pair_is_assigned, i,
                  propagation_passes, interval_tolerance,
                  validation_tolerance);
          lo_work = propagated.1;
          hi_work = propagated.2;
        }

        lo_direct = lo_work[i, j];
        hi_direct = hi_work[i, j];
        lo = fmax(-1.0, lo_direct);
        hi = fmin(1.0, hi_direct);
        ...
        if (is_free == 1) {
          ...
          // Intersect with up to lookahead_span future projections. A span
          // of zero disables localized look-ahead; span >= K means all.
          if (j < lookahead_end) {
        ...
        // Update the same localized horizon used to constrain x above.
        if (j < lookahead_end) {
          for (k in (j + 1):lookahead_end) {
...
data {
  ...
  int<lower=0, upper=K> lookahead_span;       // 0=off, K=all future pairs
  int<lower=0, upper=8> propagation_passes;   // symmetric all-triple sweeps
}
...
transformed parameters {
...

  {
    tuple(matrix[K, K], real) transform = cvine_bound_equality_jacobian(
        raw, corr_lower, corr_upper, lookahead_span, propagation_passes,
        1e-12, 1e-10);
    Omega = transform.1;
