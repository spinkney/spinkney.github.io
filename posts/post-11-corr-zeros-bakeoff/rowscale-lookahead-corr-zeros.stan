functions {
  ...
  */
  ...
  matrix cholesky_corr_constrain_rowscale_lookahead_jacobian(
    ...
    for (i in 2:K) {
      ...
      // As in rowscale, the untouched tail stores the remaining row radius.
      ...
      for (j in 1:(i - 1)) {
        ...
        */
        ...
        if (current_is_zero) {
          ...
          L[i, j] = -b1 / L_jj;
          // Delta correction from Omega[i,j] to the direct L[i,j] entry.
          jacobian += -log(L_jj);
          zero_idx += 1;
        } else {
          // Translate the propagated ordinary-correlation interval through
          // Omega[i,j] = b1 + L[i,j] * L[j,j]. Sampling L directly means
          // the transformed bound width already contains the 1 / L[j,j]
          // scale, so there is no additional free-cell scale Jacobian.
...
transformed parameters {
  matrix[K, K] L_Omega =
      cholesky_corr_constrain_rowscale_lookahead_jacobian(
