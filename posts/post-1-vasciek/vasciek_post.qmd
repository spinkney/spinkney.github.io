...
fit_stan_vector |>
  ...
  stat_slabh(fill = blog_colors$teal) +
  theme_blog() +
  labs(title = "Posterior distribution of alpha by quantile")

fit_stan_vector |>
  spread_draws(beta[K]) |>
  ggplot(aes(y = K, x = beta))  +
  stat_slabh(fill = blog_colors$purple) +
  theme_blog() +
  labs(title = "Posterior distribution of beta by quantile")
```

In my next post, [quantile regression post](/posts/post-2-quantile-reg-series/post-2-quantile-reg-part-I/quantile-reg.html) I'll show various ways to represent a quantile regression in Stan where the outcome, however, is continuous on the real line.
